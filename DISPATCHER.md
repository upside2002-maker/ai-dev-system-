...
- **Голос «Аида» (→ Owner):** живой тест №1 (2026-06-10): ключ — 3/3 ✅; «стоп» один раз сработал, затем через ~25 мин завис локальный whisper-server и стоп оглох (он на нём жил), зависшие проверки блокировали ворота. Этап 1.1 закрыт офлайн: стоп-команда переведена на vosk (корпус 21/21 стопов + 18/18 фоновых), сторож перезапускает зависший whisper на лету, гонка «cancel после конца ответа» глотается, 87/87 тестов, Reviewer ACCEPT + внесена его однострочная правка безопасности перезапуска. **Нужно слово Owner'а «запускай» на повторный живой тест** (ключ + стоп несколько раз + выдержка >25 мин). Известный наклон настройки: слова на «сто-/стой-» поверх её речи могут зря оборвать ответ («лучше лишний раз замолчит») — следим вживую. ТЗ §9: `aida-voice-bridge/WAKE_SPOTTER_TZ.md`, статус: `project-overlays/aida-voice/STATUS_RU.md`.
