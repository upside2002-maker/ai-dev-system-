...
|------|-----|--------------|
| Admin | главный (Owner здесь) | **голос «Аида»: этап 1 «надёжный ключ» закрыт офлайн 2026-06-10** (Worker + Reviewer ACCEPT, две итерации) — ждёт живого теста Owner'а; дальше Phase 2 (подключение к хозяйству) |
...
- **Голос «Аида» (→ Owner):** этап 1 — надёжный ключ (Vosk вместо whisper-проверки) — закрыт офлайн: 71/71 тестов, на озвученном корпусе 18/18 обращений поймано и 51/51 фоновых отсечено, ложных ноль; Reviewer ACCEPT, рекомендует живой тест. **Нужно слово Owner'а «запускай»** (~10 минут: десяток «Аида…», фоновый разговор, «стоп» во время длинного ответа). Запуск и глушение процесса — на Admin'е по протоколу передачи. После живого ОК — Phase 2 (подключение к хозяйству). ТЗ: `aida-voice-bridge/WAKE_SPOTTER_TZ.md`, статус: `project-overlays/aida-voice/STATUS_RU.md`.
