...
Дата последнего обновления: 2026-06-06 (Возвраты планет — **независимый ревью пройден, долг X-1 закрыт**. Движок считает верно, нашли один мелкий баг на краю — заведён отдельной задачей. Сам модуль готов и работает у Марины.)

## Сейчас

**Возвраты планет — проверены независимо, можно доверять.** Свежий ревьюер (отдельная сессия, не тот, кто писал) перечитал весь код возвратов с нуля, сам прогнал тесты (всё зелёное), подтвердил: математика верная, движок реально доходит до карточки клиента и PDF. Этим закрыт процессный долг — раньше я проверял эту работу сам, а для денег/математики так нельзя. На будущее поставлен замок: такие задачи нельзя закрыть без отдельного проверяющего.

**Один найденный баг (мелкий, заведён отдельно):** если спросить возвраты ровно в тот момент, когда человек *прямо сейчас в середине* своего возврата Сатурна/Юпитера — подписи проходов сбиваются и хвост текущего возврата склеивается со следующим (через ~29 лет). Даты и направление при этом всегда верные. На обычном «посмотреть на сегодня» для большинства не проявляется. Задача `2026-06-06-returns-retro-loop-midref-numbering` (низкий приоритет, ждёт очереди).

**🎯 ЭПИК «ВОЗВРАТЫ ПЛАНЕТ» (returns-MVP) — ЗАВЕРШЁН end-to-end 2026-05-24.** Пять фаз закрыты: A (Stage 0 эмпирический proof) → B (`Domain.Returns` + вынос `Domain.TransitMath`) → C (schema-gate: `returns-*` контракт + `Bridge.Returns` + Main dispatch) → D (Python sampler + `GET /persons/{id}/returns`) → E (UI-панель + PDF-секция). Человек/Марина теперь видит ближайшие возвраты всех 10 планет: live UI (от сегодня) + секция в соляр-PDF (от даты рендера). Геоцентрика, орб 1°, fast=один контакт / slow=серия ретро-проходов, внешние с пометкой. Вся математика в Haskell core; Python только эфемериды+endpoint+PDF; контракт JD (конвертация в даты — презентация). Параметры выведены из первоисточника (Дараган «Транзиты», прислан Мариной). Мемо: `ARCHITECTURE/planet-cycles-module-architecture-2026-05-24.md`.

**Roadmap (НЕ в MVP, будущие волны методологии Марины):** волна 2 — самоаспекты (Сатурн квадрат Сатурн, Уран оппозиция Уран ~42 и т.д.; потребует Correction 010 — квинконс scope +PlanetCycles); волна 3 — прохождение по куспидам домов (проработка сфер); волна 4 — глубокие трактовки (Дараган-стайл парафраз, решить гард «≤2 предложения»/no-verbatim); волна 5 — Lilith (Дараган возвышает: must-check; нужно решение **mean/true** + расширение `data Planet`) + Узлы (Дараган: возвраты Узлов не использовать).

**TASK planet-returns-ui-pdf — CLOSED 2026-05-24 (Tier B mixed UI+PDF presentation; ACCEPTED по TL inline-verify APPROVE + живой осмотр; product `51f1e57`).** Phase E финал. **Проверено:** scope = 9 presentation-файлов (PDF: `app/pdf/returns_section.py` new + `solar.html.j2` + `builder.py` + 2 теста; UI: `ReturnsPanel.tsx` new + `api.ts getPersonReturns` + `lib/i18n.ts formatJdDate` + `PersonDetails.tsx` mount); schema/Haskell/Phase-D **untouched** (`git diff 4bdf1b1..51f1e57` по этим путям пуст); `pytest test_returns_section` **13/13** воспроизведено, full suite **728/3/0**; PDF reuse Phase D `build_returns_snapshot`+`run_core_analysis` напрямую (ОДИН вызов, не HTTP). **Живой рендер Марины cons 15 (TL eyeball):** секция «Ближайшие возвраты планет» присутствует — Солнце 22 марта 2027 (с 21 по 23, «раз в год ≈ ДР»), Луна «раз в месяц», Марс «раз в два года», Юпитер «раз в 12 лет», Сатурн серия «проходы: …» + «раз в 29–30 лет», Уран «раз в жизнь (~84 года)» + серия, Нептун/Плутон «вне продолжительности жизни» + серия; intro объясняет возврат + серию медленных; **существующие 6 секций целы** (транзит-календарь/темы на месте). Факт-таблица, без Дараган-воды (трактовки = волна 4). Презентация: дата «22 марта 2027» (родит. месяц, единая UI+PDF), окно «с X по Y» (для быстрых схлопывается в 1 день), серия = headline + «проходы:» + окно на всю петлю, группировка 6-key→Меркурий→внешние, секция перед «Справочные данные», UI-панель в PersonDetails (gate на birth_timezone). **0 STOP.** Closure cascade: TASK review→done (archive), HANDOFF closed (archive).

**TASK planet-returns-ui-pdf — Phase E видимый слой landed 2026-06-05 (Tier B presentation; product `51f1e57` на main; Reviewer = TL inline-verify + живой осмотр PDF/UI; ФИНАЛ эпика возвратов).** Phase E (последняя) эпика «возвраты планет»: две клиентские поверхности для standalone-калькулятора «ближайшие возвраты планет». **Один product commit `51f1e57` (9 файлов, +1063/-1):** PDF — (1) `services/api-python/app/pdf/returns_section.py` NEW: `build_returns_section(person, render_jd)` **reuse Phase D** `build_returns_snapshot`+`run_core_analysis` (ОДИН snapshot, ОДИН subprocess — bright lines #6/#7, НЕ HTTP); JD→дата «22 марта 2027» через общий `_jd_to_utc` билдера (math не дублирован); группировка 6-ключевых (Луна/Солнце/Венера/Марс/Юпитер/Сатурн) → Меркурий вторично → внешние с пометками; медленные = серия проходов (headline=ближайший, окно=вся петля, проходы списком); graceful degradation. (2) `solar.html.j2` — секция «Ближайшие возвраты планет» **перед «Справочные данные»** (reference=дата рендера `render_jd_now()`). (3) `builder.py` — `_render_jd_now()` + регистрация Jinja-globals; lazy import (circular-dep guard). UI — (4) `api.ts` `getPersonReturns(id, asOf?)` (по образцу `geocodePlace`, live default=now). (5) `components/ReturnsPanel.tsx` NEW — таблица-зеркало PDF; JD→дата в TS через новый `formatJdDate`. (6) `lib/i18n.ts` `formatJdDate` (reuse `jdToUtc`). (7) `PersonDetails.tsx` — панель-карточка (gate на `birth_timezone`). Тесты — (8) `tests/test_returns_section.py` NEW (13 тестов: формат даты/окна, fast vs slow серия, группировка, пометки внешних, drop unknown-body, e2e Марина, degradation без tz). (9) `tests/test_api_pdf_endpoint.py` — page-count bound 23→25 (одна добавленная страница; существующие секции не тронуты). **Презентационные решения (на eyeball TL):** формат даты «22 марта 2027» (genitive, единый UI+PDF); окно «с X по Y» (схлопывается в один день у быстрых); серия медленных = headline-строка + «проходы:» подстрока + окно на всю петлю; секция перед справочными. **Живой рендер Марины cons 15 (verbatim из rendered HTML):** Солнце 22 марта 2027; Сатурн 23 февраля 2048 [проходы: 23.02/28.06/21.11.2048] окно с 12 фев по 1 дек 2048; Нептун/Плутон «вне продолжительности жизни». **Все 8 .section-title в порядке** (6 существующих + новая перед «Справочные данные») → существующие секции целы. **Верификация:** `pytest --tb=short -q` **728 passed / 3 skipped / 0 failed** (715 baseline + 13); `tsc --noEmit` clean (exit 0); `cabal test` **279/0** (Haskell не тронут). **0 STOP triggers** (no ISO в контракте; no math dup; no Haskell/contract/Phase-D правок; no Daragan verbatim — факт-таблица; PDF зовёт функцию, не HTTP; существующие секции целы). Out-of-scope артефакты (`.claude/launch.json`, `_marina-deliverables/`, `marketing-site/`) НЕ закоммичены: staged только 9 returns-файлов; `git status --short` чист для intended. HANDOFF: `HANDOFFS/2026-05-24-worker-to-tl-planet-returns-ui-pdf.md`. **Reviewer: TL inline-verify + живой осмотр PDF/UI** (client-facing). Открытые (не блокеры): Daragan-трактовки = волна 4 (отложены); Lilith pending mean/true; Jupiter рендерится single-pass (data-driven, в окне Марины один проход — код поддерживает обе формы). Следующее: **TL eyeball живого PDF/UI** → закрытие эпика возвратов A-E; затем волны 2-5 (самоаспекты/куспиды/трактовки/Lilith) по roadmap мемо § 9.

**TASK planet-returns-python-endpoint — CLOSED 2026-05-24 (Tier B Services; ACCEPTED по TL inline-verify APPROVE; product `4bdf1b1`).** Phase D эпика возвратов. Reviewer был optional (Tier B, математика делегирована в Core) → TL inline-verify. **Проверено:** scope = 4 Python-файла (`app/ephemeris/returns_sampler.py` new, `app/api/returns.py` new endpoint, `app/main.py` +4 router, `tests/test_returns_endpoint.py` new); **ОДИН subprocess-вызов** (`returns.py:148 run_core_analysis(snapshot)` — нет per-planet серии, bright line #6); **нет математики возвратов в Python** (sampler только сырые (jd,lon,speed); все grep-«crossing/orb/aspect» — комментарии «no … just raw samples»; детекция в Haskell, bright line #7); `pytest tests/test_returns_endpoint.py` **7/7** воспроизведено, full suite **715/3/0**. Endpoint `GET /persons/{id}/returns?as_of=<iso>` (default now) — standalone per-person, **не пишет в БД** (зависит от as_of). Phase A parity на Марине: Sun pass 1 ≈ 2027-03-22 (Δ~5с от Phase A), Saturn 3-pass серия, Neptune `beyond_lifespan=true`. Payload Марины 10 009 sample'ов / ~820 KB (логируется, <20k/<3MB guard). Геоцентрика тропик `FLG_SWIEPH|FLG_SPEED` (не sidereal). **0 STOP.** Closure cascade: TASK review→done (archive), HANDOFF closed (archive). Следующее: **Phase E** (UI-панель «Ближайшие возвраты» live от сегодня + PDF-секция reference=дата рендера + JD→дата конвертация + презентация: акцент на 6 ключевых, пометки внешних). **Lilith pending** (mean/true).

**TASK planet-returns-python-endpoint — Phase D Python sampler + endpoint landed 2026-06-05 (Tier B / normal; product `4bdf1b1` на main; Reviewer optional — Services, математика делегирована в Core).** Phase D эпика «возвраты планет»: Python-слой поверх Phase B движка + Phase C `returns`-контракта. **Один product commit `4bdf1b1` (4 файла, +721):** (1) `services/api-python/app/ephemeris/returns_sampler.py` NEW — `build_returns_snapshot(person, reference_jd)`: натальные позиции 10 планет через **reuse** `bridge.compute_planet_positions` (та же конвенция `FLG_SWIEPH|FLG_SPEED`, тропик, геоцентр — sidereal НЕ вводился) + per-planet forward sample'ы по окнам/каденции мемо § 4 (`_RETURNS_SAMPLE_PLAN`: Луна 35д/6ч, Солнце/Меркурий/Венера 400д/сутки, Марс 800д/сутки, Юпитер 13л/неделя, Сатурн 30л/10д, Уран 85л / Нептун 166л / Плутон 249л / месяц) → сборка `returns-input` JSON; `reference_jd_from_iso(as_of)` (ISO date/datetime, 'Z' ок, naive→UTC; None→now). **НИКАКОЙ математики возвратов/аспектов/пересечений** (bright line #7). lat/lon НЕ нужны (возврат = соединение по долготе, observer-independent). (2) `services/api-python/app/api/returns.py` NEW — `GET /persons/{id}/returns?as_of=<iso>` (default now): person → reference_jd → sampler → **ОДИН** `run_core_analysis(workflow=returns)` (reuse существующего solar-helper'а, аддитивно, без правок) → returns-output как есть (JD остаётся JD; конвертация в даты — Phase E). 404 missing person; 422 missing tz / invalid as_of; 502 core fail. **В БД НЕ пишется** (зависит от as_of → протухнет). (3) `app/main.py` — регистрация `returns_router` под `/api/v1` (по образцу geocode, +2 строки). (4) `tests/test_returns_endpoint.py` NEW (7 тестов): sampler валиден против `returns-input.schema.json`; endpoint 200 + 10 планет + валиден против `returns-output.schema.json`; **Phase A parity** (Sun ≈ ДР 22.03.2027, Saturn серия pass_number 1..N с Direct+Retrograde, Нептун/Плутон `beyond_lifespan=true`, Уран `false`, быстрые = 1 контакт); payload-guard (<20k sample'ов / <3MB, лог проверен); default as_of=now; invalid as_of 422. **Один subprocess-вызов — доказано:** единственный `run_core_analysis` в `get_person_returns`, helper делает один `subprocess.run`, без цикла/второго вызова. **Верификация:** `pytest --tb=short -q` **715 passed / 3 skipped / 0 failed** (708 baseline + 7 returns; 3 skip pre-existing); `cabal test` не затронут (Haskell не менялся). **Spot-check person 4 (Марина) `as_of=2026-05-24`:** 10 планет, 18 записей (быстрые=1, Saturn/Uranus/Neptune/Pluto=3-pass петли); Sun `exact_jd`=2461486.81 ≈ 2027-03-22 07:29 (Δ ~5s от Phase A); Saturn pass 2 Retrograde 2469255.70 ≈ 2048-петля; Neptune `beyond_lifespan=true` 2507482.50. Payload Марины: **10 009 sample'ов / ~820KB** (per-planet залогирован). **Девиаций от scope нет.** **0 STOP triggers fired** (schema/Haskell/Main.hs/UI не тронуты; математики в Python нет; один вызов; БД не кэшируется; sidereal нет). Out-of-scope артефакты (`.claude/launch.json`, `_marina-deliverables/`, `marketing-site/` — отдельная маркетинговая работа) НЕ закоммичены: staged только 4 returns-файла; `git status --short` чист для intended changes. HANDOFF: `HANDOFFS/2026-05-24-worker-to-tl-planet-returns-python-endpoint.md` (subprocess-proof + sample-вывод + payload + self-review). **Reviewer optional (Tier B Services) — TL inline-verify достаточно.** Следующее: **Phase E** (UI-панель «Ближайшие возвраты» live от сегодня + PDF-секция reference=дата рендера + JD→дата конвертация).

**TASK useful-people-human-readability-polish — CLOSED 2026-05-24 (Tier B− text polish; ACCEPTED по user «закрой polish как есть» = ack на 5 Worker-drafted фраз; product `ba806d5`).** User закрыл as-is 2026-05-24, подтвердив 5 фраз для Moon/Jupiter/Saturn/Uranus/Neptune (люди тёплые-чуткие / широкие-оптимистичные / надёжные-выдержанные / независимые-нестандартные / чувствующие-творческие). Стадии 1-5: `_join_sign_people` (Oxford-join «Водолеи, Рыбы и Овны»), `_FIRST_HOUSE_PLANET_PEOPLE_RU` rewrite (5 user-given + 5 Worker-drafted, без жаргона), `[:2]` cap + test, 3-sentence template сохранён. **Pytest 705/3/0 (+8 new). Cabal clean. 0 STOP.** Closure cascade: TASK review→done (archive), HANDOFF closed (archive). Reviewer был optional (Tier B− text); user-ack заменил. (Non-blocking Reviewer-comments от Intercepted-Sign — dead computation co_rulers + cosmetic — по-прежнему не трогаем.)

**TASK planet-returns-schema-gate — CLOSED 2026-05-24 (Tier A / strict; ACCEPTED по TL-inline Reviewer APPROVE; product `37db1c2`).** Reviewer инлайн (субагент-Reviewer ранее упёрся в session limit — прецедент). **Воспроизведено независимо:** атомарность — все 5 артефактов в одном commit `37db1c2` (13 файлов, `git show --stat`); `cabal test` **279/0** (returns golden + input roundtrip); `pytest test_contracts` **8/8** (5+3 returns); **solar untouched** — `git diff a1e0c75..37db1c2` по solar-схеме/golden/fixtures пуст; `Bridge.Solar` diff = +2 строки (экспорт `planetFromText`, additive, без логики); JD-конвенция в схеме (`exact_jd`/`orb_*_jd` числа, явно «NOT ISO»); DTO reuse (Bridge.Returns импортирует `BridgePlanetPosition`/`BridgeTransitSample`, не дублирует — Correction 001); `motion` enum PascalCase через `$ref MotionPhase` (reused из solar — девиация от lowercase-наброска корректна, иначе дивергентный enum); схема↔Bridge↔TS byte-consistent (поля 1:1, nullable orb); scope только schema-gate surface (нет Phase D/E creep). **0 STOP triggers. Вердикт: APPROVE, ship as-is.** Closure cascade: TASK review→done (archive), HANDOFF closed (archive). Следующее: **Phase D** (Python sampler per-planet окна § 4 + endpoint `/persons/{id}/returns`), затем **Phase E** (UI-панель + PDF-секция). **Lilith pending** (mean/true до волны Lilith).

**TASK planet-returns-schema-gate — Phase C schema-gate landed 2026-06-05 (Tier A / strict; product `37db1c2` на main; bright line #8 — ВСЕ 5 артефактов в ОДНОМ commit; Reviewer REQUIRED — Tier A schema-gate).** Phase C эпика «возвраты планет»: новый **standalone `returns` workflow** проведён через границу процесса (greenfield контракт + Bridge DTO + dispatch + golden + contract-test + TS). **5 артефактов (один commit `37db1c2`):** (1) `packages/contracts/returns-input.schema.json` + `returns-output.schema.json` (Draft-2020-12, `additionalProperties:false`, **JD-конвенция** `exact_jd`/`orb_enter_jd`/`orb_exit_jd` как числа — НЕ ISO; `motion` enum = `MotionPhase` PascalCase `Direct|Retrograde|DirectReturn`, как у solar `phase`); (2) `core/astrology-hs/src/Bridge/Returns.hs` (new: `ReturnsResolvedInput`/`ReturnsComputedFacts`/`runReturns`, оборачивает Phase B `Domain.Returns.findNearestReturns`; **reuse** `BridgePlanetPosition`/`BridgeTransitSample`/`BridgeMeta`/`planetFromText` из `Bridge.Solar` — без дублирования, Correction 001) + `app/Main.hs` dispatch `returns` (solar НЕ тронут; unknown → exit 2) + `Bridge.Solar` аддитивный экспорт `planetFromText` (runSolar+DTO байт-в-байт) + `.cabal` регистрация; (3) `test/golden/returns-sample.{input,expected}.json` (реальные Swiss-Eph sample'ы: Sun+Mars fast single, Saturn 1971-натал → 3-pass ретро-серия Direct/Retrograde/DirectReturn) + `Test.GoldenReturns` (parse→runReturns→expected + input roundtrip); (4) `packages/test-fixtures/returns-sample.json` (зеркало) + `tests/test_contracts.py` (check_schema ×2 + validate + flat-list/JD asserts); (5) `apps/web-react/src/types.ts` (`PlanetReturn` + `ReturnsComputedFacts`). **`returns[]` плоский: медленные = серия pass_number, быстрые = одна запись.** **Верификация:** `cabal build` clean + `cabal test` **279 examples, 0 failures** (baseline 275 + 4 returns golden) + **0 правок** solar schema/golden/fixtures (`git diff a1e0c75..37db1c2` по solar-путям пуст); `pytest tests/test_contracts.py` **8 passed** (5 baseline + 3 returns), full suite **708 passed / 3 skipped**; `tsc --noEmit` clean. runSolar/returns/unknown dispatch проверены end-to-end через CLI. **0 STOP triggers fired.** Схема↔Bridge↔TS взаимно согласованы (поля 1:1, `motion` enum идентичен, nullable `orb_*` = `["number","null"]`/`number|null`/`Maybe Double`). HANDOFF: `HANDOFFS/2026-05-24-worker-to-tl-planet-returns-schema-gate.md` (Reviewer-Ready section + self-review checklist). **Девиация от черновика TASK:** `motion` casing — TASK-набросок схемы показывал lowercase `direct|...`, но locked-инструкция «match Bridge.Solar exactly» → PascalCase (иначе дивергентный enum + поломка reuse); flagged для TL. **Reviewer REQUIRED (Tier A schema-gate) — TL спускает внешнего Reviewer или ревью инлайн.** Следующее: Phase D (Python sampler + endpoint), Phase E (UI-панель + PDF-секция).

**TASK planet-returns-core-domain — CLOSED 2026-05-24 (Tier A / strict; ACCEPTED по TL-inline Reviewer APPROVE; product `a1e0c75`).** Внешний Reviewer-субагент упёрся в session limit → TL провёл независимое ревью инлайн (устоявшийся прецедент). **Воспроизведено независимо:** `cabal build` clean; `cabal test` **275 examples, 0 failures**; golden `synthetic-solar-1` ✔ + **0 правок** `*.expected.json` (`git diff` пуст) → bit-identical инвариант B.1 держится. Вынос — реальный refactor: хелперы (`signedArc`/`findCrossings`/`interpolateOrbBoundary`/`findOrbWindows`/`phaseFromSpeedAndPass`/`numberPasses`) экспортируются ТОЛЬКО из `TransitMath`, в `TransitCalendar` не переопределены (нет дублирования math; bright line #7 / Correction 004). `findNearestReturns` корректен: fast=`take 1`, slow=серия, орб 1°, `jd > referenceJd` (строго после; contact-at-reference исключён тестом), `beyond_lifespan` только Нептун/Плутон. `findSolarReturnJd` только в комментарии (не вызывается); нет wildcard; `data Planet` = 10 конструкторов; все классификаторы перечислены явно (Correction 002). Ретро-семантика `[Direct, Retrograde, DirectReturn]`; Saturn cross-check headline ≈ 2048-02-23 ±1 дн от Phase A — реальная валидация (не тавтология). Scope: 6 Core-файлов, никаких schema/Bridge/Main/Python/frontend. MotionPhase-девиация (вместо чернового `prMotion::TransitPhase`) — каноничный тип, корректна. **0 STOP triggers. Вердикт: APPROVE, ship as-is.** Closure cascade: TASK review→done (archive), HANDOFF closed (archive). **Открытые вопросы Phase C+** (не блокеры): headline внешних = pass 1 (серия тоже эмитится); action-window тесный орб vs полный span петли (Phase D/E). **Lilith pending** (Дараган возвышает; мемо § 0.2) — решить mean/true до волны Lilith. Следующее: Phase C (schema-gate, greenfield `returns-*.schema.json`).

**TASK planet-returns-core-domain — Phase B Core движок возвратов landed 2026-06-05 (Tier A / strict; product `a1e0c75` на main; backup pushed `ba806d5..a1e0c75`; Reviewer REQUIRED — Core math).** Phase B эпика «возвраты планет»: pure Core-движок ближайших возвратов 10 классических планет, **один product commit**. **B.1 — вынос `Domain.TransitMath`** (new): retro-aware ядро детекции (`CalSample`, `signedArc`, `interpolateZero`, `interpolateOrbBoundary`, `findCrossings`, `findOrbWindows`, `phaseFromSpeedAndPass`) вынесено из `TransitCalendar.hs` + новый record-agnostic kernel `numberPasses` (sort-by-jd + `zip [1..]` + phase — раньше inline в `numberContactsFor`). `TransitCalendar.numberContactsFor` теперь делегирует нумерацию в `numberPasses` → math нумерации в ОДНОМ месте (bright line #7 / Correction 004, без копипаста). **Инвариант соблюдён: `analyzeAnnualCalendar` бит-в-бит** — golden `synthetic-solar-1` + все `TransitCalendar` кейсы проходят БЕЗ правок expected-файлов (0 edits, `git status` без `*.expected.json`). **B.2 — новый `Domain.Returns.findNearestReturns`** per § B.2: для каждой натальной планеты — `findCrossings` натальной долготы (retro-aware), drop ≤ reference_jd (строго будущее), `numberPasses`, окно орб 1° через `findOrbWindows`. Быстрые (Sun/Moon/Mercury/Venus/Mars) → ближайший единственный; медленные (Jupiter..Pluto) → вся ретро-петля (headline = pass 1) с pass_number + Direct/Retrograde/DirectReturn. `beyond_lifespan` = True только Нептун/Плутон (Уран once-in-life = False). Порядок Sun..Pluto. **НЕ использован `findSolarReturnJd`; нет wildcard в Planet-match (Correction 002); `data Planet` не расширен.** Naming: TASK черновик называет поле `prMotion :: TransitPhase`, но канонический тип в проекте — `MotionPhase` (тот же, что у `TransitContact.tcPhase`); использован `MotionPhase` без синонима. **B.3 — `Test.Domain.ReturnsSpec`** (33 теста): синтетика прямого крестика (exact JD < 1 мин), синтетика 3-pass ретро-петли (pass 1/2/3 + фазы), окно орб 1°, reference boundary (строго после, contact-at-reference исключён), `beyond_lifespan`, детерминизм + порядок Sun..Pluto, и **realistic cross-check Сатурна Марины** (sample'ы 2048-петли сгенерированы one-off через pyswisseph venv `FLG_SWIEPH|FLG_SPEED` geocentric, hardcoded в тесте — Core-only) → pass 1 ≈ **2048-02-23** (±1 дн от Phase A 2048-02-23 05:58), pass 2 ретро ≈ 2048-06-28, pass 3 DirectReturn ≈ 2048-11-22. **`cabal build` clean (0 warnings под `-Wall -Wcompat -Wredundant-constraints`); `cabal test` 275 examples, 0 failures (242 baseline + 33 new). 0 STOP triggers fired.** HANDOFF: `HANDOFFS/2026-05-24-worker-to-tl-planet-returns-core-domain.md` (Reviewer-Ready section). Открытые вопросы для TL (не блокеры, Phase C+): headline для внешних = pass 1 (реализовано, серия тоже эмитится); action-window — тесный орб 1° per pass vs полный span петли (Phase D/E). **Reviewer REQUIRED (Tier A Core math) — TL спускает внешнего Reviewer.**

**TASK planet-returns-stage0-empirical-proof — Phase A Stage 0 эмпирический proof выполнен 2026-06-05 (READ-ONLY анализ через pyswisseph; продуктового кода НЕТ; gate для Phase B Core `Domain.Returns`).** На реальной карте Марины (person_id=4, consultation 15) вычислены ближайшие возвраты всех 10 планет от reference_date=2026-06-05Z. Эфемеридная конвенция совпадает с движком (`FLG_SWIEPH|FLG_SPEED`, тропик, геоцентр; sidereal НЕ вводился); натальная сверка facts_json↔pyswisseph = **0.00 arcsec** для всех 10. **Все sanity STOP-критерии PASS:** Sun-return 2027-03-22 Δ=+0.31 дн (≤±2); Moon nearest +16.72 дн, шаг между возвратами 27.34 дн = сидерический месяц; Saturn age 58.92 = 2×29.46 (второй возврат); Uranus 83.90 ≈ 84 (раз в жизнь); Neptune 163.92 / Pluto 246.72 — за пределами жизни (`beyond_lifespan` оправдан). **Retro-кейс подтверждён на РЕАЛЬНЫХ данных** (синтетика не понадобилась): Saturn/Uranus/Neptune/Pluto пересекают натальную долготу 3× в петле direct→retro→direct (Saturn loop 272.8 дн; pass 3 = `DirectReturn`). **Вывод по reuse:** `SolarReturn.findSolarReturnJd` НЕ переиспользуется — его forward-only `crossesArc` доказанно ПРОПУСКАЕТ реальное ретро-пересечение Сатурна (forwardDelta=359.85° vs offsetNat=359.91° → False); в `Domain.TransitMath` выносятся 6 retro-aware хелперов из `TransitCalendar.hs` (`signedArc`, `findCrossings`, `interpolateOrbBoundary`, `findOrbWindows`, `numberContactsFor`, `phaseFromSpeedAndPass`). **0 STOP triggers. Продуктовый репозиторий чист.** HANDOFF: `HANDOFFS/2026-05-24-worker-to-tl-planet-returns-stage0-empirical-proof.md`. Открытые вопросы для TL (не блокеры): nearest-return для внешних = pass 1 vs pass 3 (`direct_return`); ширина action-window (петля ~9-10 мес vs тесный орб ±1°).
...
**User feedback 2026-05-20 verbatim:** «Это правильное разделение слоёв: психологический уровень теперь про внутреннее переживание транзита, событийный — про дома и сферы. То, что Reviewer проверил все 11 generic-карт Ольги, а не только 3, хорошо закрывает риск.»

**Reviewer non-blocking suggestions logged (NOT scheduled, future-polish candidates):**
- **Same-transit opener repetition** (4 Olga Uranus carts начинаются identically «Внутри поднимается потребность в свежести и свободе...»): by design (deterministic composition; current acceptance `test_psychology_distinct_per_aspect` requires distinct **по aspect**, не по opener variety). **User direction 2026-05-20: accepted as non-blocker.** Contingency: «Если Марина будет читать подряд и скажет "однообразно", тогда откроем маленький variant-pool TASK.» Future variant-pool TASK candidate (Tier C, per-planet 2-3 opener variants с deterministic hash-based selection per (transit_planet, target) tuple).
- HANDOFF mentions 6 cards while real PDF renders 11 — tests auto-adapt (iterate full collection без hardcoded count); non-blocker.
- HANDOFF inventory documentation could be tightened — coverage works fine.
