...
Внутренний инструмент Марины для подготовки соляр-консультаций. **Программа Transit Section Recovery — CLOSED 2026-05-15.** **3 post-recovery follow-up TASKs CLOSED 2026-05-16** (API PDF endpoint + directions full-list + transit section generic-output). **Phase 9.0 — Marina Significance Selection Analysis** delivered 2026-05-17 — analytical memo only, no product code changes. **Phase 9.1 — Directions Filter — STOP at Stage 0 (2026-05-17)** — Worker honoured user direction «9.1 должен строго опираться на явно найденное правило Марины, а не на новый heuristic soup. Если rule A2/A3 начнут спорить с текстом Марины — Worker обязан STOP, не "улучшать" правило молча». Stage 0 empirical validation (4 filter variants × 8 calibrated cases + Ольга) показал что **никакая формулировка A1+A2+A3 не воспроизводит Marina selection одновременно** для Ольги (4/9) И калиброванных кейсов (Marina-selected transpersonal-source directions в cases 01/05/09/10). V2 (A1=target∈S OR directed∈ANGLES; A2 simple-drop-transpersonal; A3 Jaccard>0.8 dedup) даёт EXACT match только для Ольги (4/9) + case 08 (2/2) — но DROPS Marina-selected directions в 4 калиброванных кейсах. A3 dedup тоже DROPS Marina-selected «Sun 60° Asc» если enter_jd ordering выбирает «Moon 90° Sun» как первый среди identical-formula pairs. Phase 9.1 escalated to TL для re-spec OR closure (HANDOFF `2026-05-17-worker-to-tl-phase-9-1-directions-filter-STOP.md`). **Зеро product code touched**; pytest baseline 368/2/0 preserved; cabal clean.
