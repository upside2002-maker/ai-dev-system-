...
Worker decides exact code placement (top-of-iteration filter vs post-aggregation filter) — pick cleanest spot.

**`ANGLE_TARGETS` constant** can live at module-level (preferred) OR inline within function.
...
- [ ] `cd services/api-python && PATH="/Users/ilya/.ghcup/bin:$PATH" .venv/bin/pytest --tb=no -q`: `(372 baseline) + 1 new property test passed (parametrized over 10 cases) + 0 xfailed + 0 failed`. **Total expected: 373 passed + 2 skipped + 0 failed** (single property test counts as 1; parametrized may produce 10 — depends на pytest convention).
