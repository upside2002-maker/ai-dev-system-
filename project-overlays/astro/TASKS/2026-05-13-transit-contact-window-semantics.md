...
- Это решает 178d shift корректно — engine output матчит Marina.
- Pros: правильно по semantic; tests pass без exceptions.
- Cons: Tier A cascade (schema, fixtures × 9 cases regen, roundtrip, contract test); может сломать что-то в других cases.
...
- 2 Neptune Cat 4 tests остаются xfail постоянно с reason: `"engine semantic vs Marina narrow display — accepted divergence, presentation aggregates to 3 windows per card"`.
- Phase 4 PDF rendering уже handles this gracefully через aggregation; user видит 3 cards с engine dates (не Marina dates), но конкретная разница не критична для интерпретации.
- Pros: zero implementation cost; reality reflected.
- Cons: не закрывается дисциплина «hard acceptance assertions» — Cat 4 xfails становятся «forever xfail», что противоречит идее Phase 2 design.
...
- new:
  - `services/api-python/app/pdf/templates/solar.html.j2` — НЕТ, нечего тут менять.
