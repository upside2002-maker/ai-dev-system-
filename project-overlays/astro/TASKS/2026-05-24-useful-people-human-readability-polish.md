...
- modify:
  - `services/api-python/app/pdf/synthesis_themes.py` (~30 lines: new join helper + axis sentences refactor + `_FIRST_HOUSE_PLANET_PEOPLE_RU` rewrite — full delete-and-replace per clarification 4).
  - `services/api-python/tests/test_consultation_summary_evidence.py` (Olga regression updated for new Mars phrasing).
  - `project-overlays/astro/STATUS_RU.md`.

- new:
  - `services/api-python/tests/test_useful_people_polish.py` (join helper unit tests + Marina regression + `[:2]` cap test per clarification 2).

- delete:
  - Old `_FIRST_HOUSE_PLANET_PEOPLE_RU` astrology-jargon entries (replaced in-place, not preserved as `_LEGACY` — per clarification 4).
...
**Ready: yes** — 4 clarifications applied 2026-05-24:

1. **Other 5 planet phrases (Moon / Jupiter / Saturn / Uranus / Neptune) = (a) Worker drafts.** Worker пишет 5 фраз в том же духе, что user-given 5 (Sun/Mercury/Venus/Mars/Pluto): без астрологического жаргона, через человеческие качества. Worker quotes drafted phrases в HANDOFF; user confirms at closure. Applied Stage 2.

2. **`[:2]` cap = (a) keep + explicit test.** Cap сохраняется. Marina (Sun+Mercury+Venus) → выводятся первые 2 (Sun + Mercury); Venus dropped silently → required test `test_useful_people_caps_first_house_at_two` fixes the behaviour as deliberate brevity guard. Applied Stage 3.

3. **3rd sentence structure = (a) keep, fix only inner phrases.** 3-клаузная форма «Также в этом году хорошо рядом — A; B; и солнечные X по той же оси» сохраняется; меняются только A/B (per Bug 2). Минимальное вмешательство, низкий risk регрессий. Applied Stage 4.

4. **Legacy `_FIRST_HOUSE_PLANET_PEOPLE_RU` = (a) delete старый dict.** Path 1 TASK закрыт, старые формулировки признаны жаргонными — rollback не ожидается. Чистый git diff. git blame trace сохраняется автоматически через commit history (старая версия видна в `git show d43e05e:services/api-python/app/pdf/synthesis_themes.py`). Applied Stage 2.
