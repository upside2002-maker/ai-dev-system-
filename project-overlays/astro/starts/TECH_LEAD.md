...
6. `/Users/ilya/Projects/ai-dev-system/BASELINE.md` — что сделала AI Dev System v0.1, какие 4 системных gap'а закрыла system-fix
7. `/Users/ilya/Projects/ai-dev-system/project-overlays/astro/README.md` — полный контекст Astro + bootstrap risks (в pack только head -40)
8. `/Users/ilya/Projects/ai-dev-system/project-overlays/astro/ARCHITECTURE/target-architecture.md` — целевая архитектура, треугольник источников истины, **8 bright lines** (§ 11) — highest priority контракты
9. `/Users/ilya/Projects/ai-dev-system/project-overlays/astro/ARCHITECTURE/migration-plan.md` — порядок миграции mini-MVP → target
10. `/Users/ilya/Projects/ai-dev-system/project-overlays/astro/ARCHITECTURE/PHASE_0_TASKS.md` — атомарные задачи Phase 0.1 (walking skeleton) + 0.2 (full)
11. `/Users/ilya/Projects/ai-dev-system/project-overlays/astro/ARCHITECTURE/current-mvp-review.md` — что физически в `/Users/ilya/Projects/astro/`, покомпонентный приговор (на момент 2026-04-24)
12. `/Users/ilya/Projects/astro/CLAUDE.md` — продуктовые правила
13. `/Users/ilya/Projects/astro/.claude/architecture-invariants.md` — bright lines (echo из target-architecture.md, локальный для продуктового repo)
14. `/Users/ilya/Projects/astro/.claude/corrections.md` — продуктовые corrections (накопленные в ходе работы над PDF и движком; включают methodology lock-ins вроде Correction 006)
...
- **Параллельная работа TL'ов — норма.** В общем репо `ai-dev-system` могут быть незакоммиченные правки TL Sitka в `project-overlays/sitka-office/...` — это не блокирует твою работу. Каждый TL коммитит **только свои файлы по явным путям**: `git add project-overlays/astro/<files>`, `git add MAILBOX/to-tl-astro.md`. **Не использовать `git add .` или `git add -A`** — они захватят чужие незакоммиченные правки. См. также `project-overlays/sitka-office/starts/TECH_LEAD.md` — там полный прецедент 2026-05-25.
- **Закрыть deliverable = git commit + push сразу.** Не оставляй сданную работу uncommitted в working tree — Owner и другие сессии её не видят, плюс блокирует параллельных TL'ов (см. правило выше). Прецеденты 2026-05-24/25: BA Astro дважды сдавал работу в MAILBOX без commit'а (Этап 1 deliverables + правки stylebook/content-plan/удаление пилота), Admin подбирал. На будущее — finish-line каждого deliverable: `git add <свои файлы> && git commit && git push`.
