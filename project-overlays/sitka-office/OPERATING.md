...
- 2026-05-14 (ночь): PR #87 автодеплой через GitHub Actions включён и протестирован → master `7b069a7`. Единственный официальный путь обновления прода теперь — workflow `Deploy to production` с manual confirm-gate. Скрипт `deploy/auto-deploy.sh` сам определяет scope, применяет миграции если есть, делает минимально-инвазивный `build + up -d --no-deps` и при провале smoke откатывает код на OLD_SHA. Owner setup завершён: 5 secrets в env `production` + private key стёрт с сервера. Required reviewers недоступен на free tier personal-account — пропущен, остаётся первый рубеж (`confirm: yes` в input). Smoke прогон на пустом diff (docs-only scope) зелёный за ~7s. Manual fallback в DEPLOY.md сохранён.
