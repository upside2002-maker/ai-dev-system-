...
- **Парсер магазинов: двухсловные запросы режут валидные товары** (наткнулся оператор 2026-05-13 на «Blizzard parka»). Файл `sitka-services/vendor/inventory_parser/inventory/v2/query.py:113` устанавливает `query_mode = "broad" if not size and not color and len(search_tokens) <= 1 else "target"`. В `target` режиме `BaseStoreAdapter.title_matches` (`adapters/base.py:452-462`) после проверки матча всех токенов **дополнительно** требует, чтобы фраза целиком (`search_phrase`) была substring в `normalized_title`. Для запроса «Blizzard parka» (2 токена → target) товары «Blizzard AeroLite Parka», «Sitka Blizzard Pro Parka» отбрасываются — между `blizzard` и `parka` стоит ещё одно слово, substring не матчится. Подтверждено probe'ом на сервере: на `blizzard` (1 токен → broad) Rogers и 1shot возвращают 5 товаров включая ту самую «Blizzard AeroLite Parka» $538.30; на `blizzard parka` (2 токена) — `no_hits` от всех четырёх магазинов. Workaround на сегодня: оператор вводит одно ключевое слово и фильтрует глазами. Предлагаемый fix Tier C ~1-3 LOC: поднять threshold с `<= 1` на `<= 2` в `query.py:113` (двусловные запросы тоже становятся broad — `title_matches` всё ещё требует **оба** токена в названии, так что мусор не пройдёт; проверено логически). Параллельно добавить unit-тесты на 2-словные запросы в `vendor/inventory_parser/tests/` (что есть сейчас — посмотреть отдельно). Затронет все магазины одновременно (общий BaseStoreAdapter).
