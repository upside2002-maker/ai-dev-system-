...
- **P1: Avito F-DUP-1 — sync rotation markers между двумя Apify-runner'ами** (из avito-аудита 2026-05-28, §6 P1-1). `vendor/avito_parser/market/avito/client.py:ZenStudioAvitoClient` (sync/urllib) ротирует Apify-токен при reason `"rate limit"` / `"too many requests"`. `vendor/avito_parser/shared/apify_runner.py:ApifyRunner` (async/aiohttp, используется в `app/inventory/adapters/stores/amazon.py` и `ebay.py`) — **не ротирует**. **Риск вырос после TASK C (2026-05-28, PR #93):** Amazon впервые работает → больше Apify-вызовов → выше шанс 429. **Если оператор словит пустую выдачу под нагрузкой — это первый подозреваемый.** Fix Tier C ~5 LOC: добавить те же markers в `_is_rotation_error` async runner'а. Не блокирует ничего сегодня, но low-cost insurance. Owner выбрал не делать в Avito Option 1 (2026-05-28), оставил в backlog.

- ~~**Парсер магазинов: двухсловные запросы режут валидные товары**~~ (наткнулся оператор 2026-05-13 на «Blizzard parka»). **Закрыто PR #89 (`ccac24b`, 2026-05-22):** в `query.py:113` порог `<= 1` → `<= 2`, двусловные запросы теперь идут в `broad` режим. Тест-регрессия — `sitka-services/tests/test_inventory_v2_query.py`. Smoke на проде после deploy: `blizzard parka` → 3 hits (rogers+1shot), главный товар со скриншота найден. **Остаточный риск (из архитектурного аудита 2026-05-21):** для 3+ словных запросов substring drop в target mode — **закрыт PR #91 (`89a6c25`, 2026-05-28, TASK B)**.
