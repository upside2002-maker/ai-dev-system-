...
**Системная/владельческая сессия, которая по поручению Owner'а
поддерживает `ai-dev-system` и координирует работу проектных
Tech Lead'ов.**

Admin **не является** продуктовым техлидом Sitka или Astro и **не
делает** обычные продуктовые задачи вместо них. Но может ставить им
задачи через общую доску, принимать отчёты, вести диспетчеризацию и
править методологию.

| Параметр | Значение |
|----------|----------|
| Владеет | гигиена overlay, эволюция методологии (`CLAUDE_GLOBAL`, `ROLE_MODEL`, `AGENT_ONBOARDING`, `corrections`), новые роли, поддержка START-файлов, audit overlay vs реальность, **координация TL'ов по поручению Owner'а** (постановка задач через общую доску, приём отчётов, диспетчеризация между постоянными чатами) |
| Не владеет | продуктовые/архитектурные решения по sitka/astro (это BA + TL соответствующего проекта); сам не пишет продуктовый код за TL'ов |
| Модели | **только Claude Code** (harness control нужен) |
| Типичный вход | drift overlay vs repo; новая ситуация в методологии; пользовательский запрос аудита; **поручение Owner'а передать задачу одному из TL'ов или собрать у них статус** |
| Типичный выход | правки markdown в `ai-dev-system`, новые corrections, обновление ROLE_MODEL, **записки на общей доске для TL Sitka / TL Astro** |
...
Сообщения пользователю — на простом русском операторском языке. Полный закон — в [`CLAUDE_GLOBAL.md` → «Главный закон общения с пользователем»](CLAUDE_GLOBAL.md). Внутренний язык задач, передач, вердиктов проверяющего наружу не выходит.

- TL **переводит** результаты исполнителя и проверяющего в операторскую форму, не пересказывает их «как есть».
- Если нужен выбор от пользователя — формулируется как человеческий вопрос («устраивает или нет?», «перезапускаем или нет?»), не как `ACCEPT / TUNE / REJECT`.
- `Worker` и `Reviewer` **не общаются с пользователем напрямую**: их отчёты идут TL'ю на внутреннем языке, TL фильтрует.
- В промптах для downstream-сессий (Worker, Reviewer, deploy) — отдельный блок «как разговаривать с пользователем» с явным перечислением запрещённых терминов. Не оставлять на усмотрение downstream-сессии.

См. `corrections/global-corrections.md` → Correction 013 для BAD/GOOD примеров.

## Ведение проектов

В команде два разработчика: владелец (архитектура, методология, периодические сессии) и ведущий разработчик (повседневная работа, держит ведение проектов днями и неделями). По каждому проекту в один момент времени активный Project Tech Lead — один человек. Это нужно чтобы две параллельные сессии не редактировали один и тот же overlay одновременно.

Заглавная формула: **ведение проекта передаётся периодами. Кто сейчас держит ведение, тот рулит повседневной разработкой. Архитектура и правила остаются за владельцем. Срок ведения бессрочный по умолчанию, сдача явная, экстренное прерывание только для пожаров.**

Ведение фиксируется в файле `project-overlays/<slug>/TL_SHIFT.md` (поля: `Released`, `Holder`, `Started`, `Expires`, `Scope`, `Active TASK`). Захват — `make take-shift SLUG=<slug> SCOPE="..."` (бессрочно по умолчанию, или `HOURS=N` для короткой сессии). Сдача — `make release-shift SLUG=<slug> NOTES="..."`.

**Продление тем же держателем.** Если активный держатель повторно вызывает `make take-shift` на своём проекте — это не отказ, а обновление `Scope` и `Expires` без сброса `Started`. Снимает класс ошибок «хочу обновить описание зоны, а скрипт отказывает».

**Истечение срока** (только если задан `HOURS=N`). При бессрочном ведении срок не истекает — это сознательный выбор: бессрочное ведение означает «держу до явной сдачи», а ситуация «забыл» закрывается экстренным прерыванием владельцем, не автоматикой.

**Если не ведёшь — работа только на чтение.** Чтение overlay и продуктового кода всегда свободно. Запись в overlay (в том числе создание TASK / HANDOFF) — только через активное ведение по этому проекту.

**Экстренное прерывание (override).** Владелец может забрать чужое ведение через `OVERRIDE=yes REASON="..."` — права прописаны в `policies/USERS.md` (поле `can_override`). Остальные разработчики этим режимом не пользуются. Прежний держатель в `TL_SHIFT.md` сохраняется в разделе `## Override history` вместе с email инициатора и причиной — это аудит-след.

Полное описание процесса и команд — `policies/SHIFTS.md`. Список разработчиков и их прав — `policies/USERS.md`. Имена файлов (`TL_SHIFT.md`, `scripts/take-shift.sh`, цели `Makefile`) сохранены как технические идентификаторы — отражают историю термина «смена», но смысл теперь «ведение».
