...
В команде два разработчика. По каждому проекту в один момент времени активный Project Tech Lead — один человек. Это нужно чтобы две параллельные сессии не редактировали один и тот же overlay одновременно. Модель — посменная: один на смене, второй на покое или работает по другому проекту.

Смена фиксируется в файле `project-overlays/<slug>/TL_SHIFT.md` (поля: `Released`, `Holder`, `Started`, `Expires`, `Scope`, `Active TASK`). Захват — `make take-shift SLUG=<slug> SCOPE="..."`, освобождение — `make release-shift SLUG=<slug> NOTES="..."`.

Default-длина смены — 8 часов. Если за это время Holder не сделал release — смена считается истёкшей, любой может взять её на себя без согласований. Это страховка от «взял и забыл».

**Если не на смене — работа только на чтение.** Чтение overlay и продуктового кода всегда свободно. Запись в overlay (в том числе создание TASK / HANDOFF) — только через активную смену по этому проекту.

**Экстренное прерывание (override).** Илья как владелец может прервать чужую смену через `OVERRIDE=yes REASON="..."` — права прописаны в `policies/USERS.md` (поле `can_override`). Остальные разработчики этим режимом не пользуются. Прежний Holder в `TL_SHIFT.md` сохраняется в виде комментария вместе с email инициатора и причиной — это аудит-след.

Полное описание процесса и команд — `policies/SHIFTS.md`. Список разработчиков и их прав — `policies/USERS.md`.
