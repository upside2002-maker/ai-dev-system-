...
for group in accept_task commit_msg self_check ledger truth_kernel action_core claim_dispatch brief ru_guard prepush contract; do
...
echo "  Добавлено: структурные заявки (tests/claim_dispatch) — фронт поверх влитых"
echo "  ядер. Диспетчер срабатывает ТОЛЬКО на поданный конверт (JSON с kind), речь"
echo "  модели в него не передаётся: НЕТ парсера свободного текста, НЕТ модели-судьи."
echo "  Пять типов маршрутизируются в влитые верификаторы (gate / cmd_action /"
echo "  run_check), новой логики проверки нет. Покрыто 10 тестов Owner'а + прямой"
echo "  ловец лживого done_report (executed-есть/мир-пуст, ветка run_check ~1238)."
echo "  Честный предел заявок (по ТЗ, чёрным по белому): НЕОБЪЯВЛЕННАЯ словесная"
echo "  опора — вне охвата ПО КОНСТРУКЦИИ (цена отказа от парсера речи). done_report"
echo "  ветка (а) run_check ГЛОБАЛЬНА (журнал↔мир целиком), а не пер-рапортна: ловит"
echo "  ложь, только если она уже в журнале как расхождение; чисто словесное «готово»"
echo "  без выставленного действия она не видит. Подстраховано постфактум (ворота"
echo "  дел, память, журнал лжи, ревью), но названо честно, не переоценено."
